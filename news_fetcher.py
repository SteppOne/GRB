...
Gira su GitHub Actions (cron): scarica il calendario, tiene solo gli eventi
High-impact in USD/EUR/GBP/AUD e scrive 'news.json'. TUTTI i bot leggono quel
JSON via NEWS_SOURCE_URL (poi ogni bot ri-filtra sulle valute che gli servono).

SORGENTI (in ordine di tentativo):
  1. ForexFactory (nfs.faireconomy.media) — GRATIS, SENZA CHIAVE API. Primaria
     dal giugno 2026: Finnhub ha tolto il calendario economico dal piano free
     (403 "You don't have access to this resource" anche con chiave valida).
     Il file e' settimanale e ruota la domenica: copre sempre le 24-48h che
     servono ai bot (nel weekend il mercato e' chiuso).
  2. Finnhub  — SOLO se FINNHUB_API_KEY e' presente E il piano la include.
  3. FMP      — SOLO se FMP_API_KEY e' presente.

Le chiavi (ora OPZIONALI) arrivano dalle variabili d'ambiente (GitHub Secrets):
...
# Il CDN di faireconomy puo' rifiutare client anonimi: User-Agent esplicito.
FF_HEADERS = {"User-Agent": "AleBotNewsFetcher/1.0"}
...
def scarica_forexfactory():
    """Primaria, gratis e senza chiave. Ritorna list (anche vuota) o None su errore.

    Formato evento FF: {"title", "country", "date", "impact", ...} dove
    'country' e' GIA' il codice valuta (USD/EUR/GBP/...) e 'date' e' ISO8601
    con offset (es. 2026-06-12T08:30:00-04:00).
    """
    try:
        resp = requests.get(FF_URL, headers=FF_HEADERS, timeout=30)
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
        print(f"ForexFactory: errore API: {e}", file=sys.stderr)
        return None
    if not isinstance(data, list):
        print(f"ForexFactory: risposta inattesa: {type(data)!r}", file=sys.stderr)
        return None
    ...
    for ev in data:
        ...
        if t is not None:
            t = t.astimezone(timezone.utc)   # normalizza l'offset locale FF in UTC
        currency = str(ev.get("country", "")).strip().upper()
...
def scarica_finnhub():
    """Fallback 1 (richiede chiave E piano col calendario). list o None su errore."""
...
def scarica_fmp():
    """Fallback 2 (richiede chiave). Ritorna list (anche vuota) oppure None su errore."""
    if not FMP_API_KEY:
...
def main():
    eventi = scarica_forexfactory()
    fonte = "forexfactory"
    if eventi is None:
        print("ForexFactory fallito -> fallback Finnhub.", file=sys.stderr)
